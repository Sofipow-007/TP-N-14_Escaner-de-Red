...
public class ComienzoScanner {
    ...
    public ComienzoScanner(String ipInicio, String ipFinal) { // Este constructor va a recibir los valores de IP inicial
                                                              // e IP final que van a servir en distintas funciones
    ...
    // Confirma si los equipos de red vinculados estan conectados o no. Si lo están,
    // se agregan al contador de equipos que respondieron.
    ...
    // Se devuelve el tiempo de respuesta en tiempo real de la solicitud del ping,
    // tanto para direcciones IP como DNS.
    ...
    public int obtenerTiempoPing(String host, int timeout) {
        try {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(process.getInputStream()))) { // Se calcula la salida del   comando ping
                String line;
                while ((line = reader.readLine()) != null) { // Se recorre línea por línea hasta encontrar un valor que
                                                             // contenga ms
    ...
    // Esta función va a devolver una lista de los equipos de red que respondan o no
    // en un rango de números identificados en dos IPs distintas.
    ...
    public listaEquiposRed escaneoEntreIPs(int timeout, Consumer<Integer> actualizarProgreso) {
        ...
        listaResultados.limpiarLista(); // Además, se actualiza la lista para cargar nuevos resultados, llamando a una
                                        // función específica de la clase listaEquiposRed()
        ...
        int base1 = Integer.parseInt(inicioPartes[0]); // base1 viene a ser el primer dígito de la IP inicial hasta el
                                                       // primer punto. Por ejemplo, si tenemos una dirección local
                                                       // (127.0.0.1) el valor sería 127

        int base2 = Integer.parseInt(inicioPartes[1]); // base2 es el segundo dígito de la IP hasta el segundo punto que
                                                       // la separa del resto de dígitos

        int base3 = Integer.parseInt(inicioPartes[2]); // base3 es el tercer dígito de la IP hasta el tercer punto

        int inicio = Integer.parseInt(inicioPartes[3]); // inicio es el cuarto dígito de la IP y con el que se va a
                                                        // calcular el contador del rango, junto con la variable "fin"

        int fin = Integer.parseInt(finPartes[3]); // fin se trata del cuarto dígito de la IP final y es el fin del rango
                                                  // para calcular los equipos e red

        if (inicio > fin) { // Si inicio es mayor a fin, devuelve una excepción ya que así no se puede
                            // calcular ningún rango
            ...
            for (int i = inicio; i <= fin; i++) {
                ...
                pool.submit(() -> { // Se recibe un Runnable de forma que se identifique en un lambda, ejecutandose
                                    // en un hilo separado
                    ...
                    synchronized (listaResultados) { // Se asegura que solo un hilo a la vez modifique la lista
                        listaResultados.agregarEquipo(new ResultadoScanner(
                                datos[1], datos[0], responde, tiempoRespuesta)); // Se crea un objeto ResultadoScanner.
                                                                                 // Se guarda en la lista
                    }

                    int hechas = procesadas.incrementAndGet(); // Suma un hilo de forma segura para poder utilizar
                                                               // múltiples hilos
                    int porcentaje = (int) ((hechas / (double) totalIPs) * 100); // Se calcula el porcentaje de IPs
                                                                                 // procesadas
            ...
            try {
                pool.awaitTermination(2, TimeUnit.MINUTES); // Le agrega al programa un tiempo de espera para que todas
                                                            // las tareas se puedan terminar
            }

            catch (InterruptedException e) { // Captura la excepción si el hilo principal que estaba esperando fue
                                             // interrumpido
                e.printStackTrace();
            }
